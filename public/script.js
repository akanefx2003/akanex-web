...
function showStatus(type, title, message) {
  ...
  document.getElementById('statusText').textContent = 
    type === 'loading' ? '⏳ En cours...' : type === 'error' ? '❌ Erreur' : '✅ Terminé';
  
  document.getElementById('statusDot').className = 
    type === 'loading' ? 'pulse-dot loading' : type === 'error' ? 'pulse-dot error' : 'pulse-dot';
}
...
function renderPlugins(plugins) {
  ...
  grid.innerHTML = plugins.map(p => `
    <div class="plugin-card" data-id="${p.id || p.name}">
      ...
      <div class="meta">
        <div class="author">👤 <span>${p.author || 'Anonyme'}</span></div>
...
document.getElementById('pluginSearch')?.addEventListener('input', (e) => {
  filterPlugins(e.target.value.toLowerCase());
});

document.getElementById('pluginFilter')?.addEventListener('change', () => {
...
async function loadStats() {
  try {
    ...
    document.getElementById('statUptime').textContent = 
      days > 0 ? `${days}j ${hours}h ${minutes}m` : `${hours}h ${minutes}m ${seconds}s`;
    
    const ram = data.memory?.heapUsed || 0;
    document.getElementById('statRam').textContent = Math.round(ram / 1024 / 1024) + 'MB';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    if (document.getElementById('tab-plugins').classList.contains('active')) loadPlugins();
